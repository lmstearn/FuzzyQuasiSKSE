...
bool IFIFO::Push(UInt8 * buf, UInt32 length)
{
	...
	if(writeOffset + length > fifoBufSize)
	{
		...
		memcpy(&fifoBuf[writeOffset], buf, segmentLength);
		memcpy(fifoBuf, &buf[segmentLength], length - segmentLength);
	}
	else
	{
		memcpy(&fifoBuf[writeOffset], buf, length);
	}
...
bool IFIFO::Peek(UInt8 * buf, UInt32 length)
{
	...
	if(fifoBase + length > fifoBufSize)
	{
		...
		memcpy(buf, &fifoBuf[fifoBase], segmentLength);
		memcpy(&buf[segmentLength], fifoBuf, length - segmentLength);
	}
	else
	{
		memcpy(buf, &fifoBuf[fifoBase], length);
	}
